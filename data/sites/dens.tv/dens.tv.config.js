...
module.exports = {
  ...
  parser({ content }) {
    ...
    response.data.forEach(item => {
      ...
      const match = title.match(/(S(\d+))?\s*(Ep|Episode)?\s*(\d+)?/i) || []
      const season = match[2] ? parseInt(match[2]) : null
      const episode = match[4] ? parseInt(match[4]) : null
  ...
  async channels() {
    ...
    for (const id_category of Object.values(categories)) {
      let data
      try {
        data = await axios.get('https://www.dens.tv/api/dens3/tv/TvChannels/listByCategory', {
          params: { id_category }
        }).then(r => r.data)
      } catch (e) {
